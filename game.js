...
function resetGame() {
  ...
  let y = game.height - 140;
  for (let index = 0; index < 35; index += 1) { addPlatform(y, index); y -= 74 + Math.random() * 39; }
  updateHud();
}
...
function addPlatform(y, index) {
  ...
  if (previous) x = Math.max(12, Math.min(maxX, previous.x + (Math.random() - .5) * 170));
...
function update(delta) {
  const player = game.player; const acceleration = 1900; const horizontalSpeed = 310;
  if (keys.left) player.velocityX = Math.max(player.velocityX - acceleration * delta, -horizontalSpeed); else if (keys.right) player.velocityX = Math.min(player.velocityX + acceleration * delta, horizontalSpeed); else player.velocityX *= Math.pow(.001, delta);
  ...
  game.platforms = game.platforms.filter(platform => platform.y < game.height + 30); while (game.platforms.length < 35) { const highest = Math.min(...game.platforms.map(platform => platform.y)); addPlatform(highest - 75 - Math.random() * 40, game.platforms.length); }
...
function drawPlayer() { const player = game.player; context.save(); context.translate(player.x + player.width / 2, player.y + player.height / 2); context.rotate(player.rotation); context.fillStyle = 'rgba(0,0,0,.3)'; context.beginPath(); context.ellipse(0, 23, 17, 4, 0, 0, Math.PI * 2); context.fill(); context.fillStyle = '#d8f546'; context.beginPath(); context.ellipse(0, -2, 16, 18, 0, 0, Math.PI * 2); context.fill(); context.fillStyle = '#10141d'; context.beginPath(); context.ellipse(0, 1, 13, 10, 0, 0, Math.PI * 2); context.fill(); context.fillStyle = '#f1eee5'; context.beginPath(); context.arc(-5, -1, 2.5, 0, Math.PI * 2); context.arc(5, -1, 2.5, 0, Math.PI * 2); context.fill(); context.fillStyle = '#ff6654'; context.fillRect(-14, 13, 10, 5); context.fillRect(4, 13, 10, 5); context.strokeStyle = '#d8f546'; context.lineWidth = 2; context.beginPath(); context.moveTo(-7, -18); context.lineTo(-10, -27); context.moveTo(7, -18); context.lineTo(10, -27); context.stroke(); context.fillStyle = '#ff6654'; context.beginPath(); context.arc(-10, -28, 2.5, 0, Math.PI * 2); context.arc(10, -28, 2.5, 0, Math.PI * 2); context.fill(); context.restore(); }
...
window.addEventListener('resize', () => { if (!game.running) resizeCanvas(); }); window.addEventListener('keydown', event => { if (['ArrowLeft', 'a', 'q'].includes(event.key.toLowerCase())) { keys.left = true; event.preventDefault(); } if (['ArrowRight', 'd'].includes(event.key.toLowerCase())) { keys.right = true; event.preventDefault(); } if (event.key.toLowerCase() === 'p') togglePause(); }); window.addEventListener('keyup', event => { if (['ArrowLeft', 'a', 'q'].includes(event.key.toLowerCase())) keys.left = false; if (['ArrowRight', 'd'].includes(event.key.toLowerCase())) keys.right = false; });
